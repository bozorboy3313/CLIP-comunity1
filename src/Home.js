      ...
      try {
        ...
        const promises = result.assets.map(async (asset) => {
          ...
          if (response.headers.get('content-type').startsWith('image/')) {
            ...
            console.error('Unsupported image format:', asset.uri);
        ...
        // Filter out null values (unsupported images)
        const validImageUrls = imageUrls.filter((url) => url !== null);

        // Update state with the new image URLs
        setImages([...images, ...validImageUrls]);
        setSelectedImages([...selectedImages, ...validImageUrls]);
      } catch (error) {
          ...
          <View style={styles.flatListItem}>
            ...
            >
              <View style={{ flexDirection: 'row' }}>
                {item.images && <Image source={{ uri: item.images[0] }} style={{ width: 80, height: 80, marginRight: 10, marginBottom: 10}} />}
                <View style={{ flex: 1 }}>
